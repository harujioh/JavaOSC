...
/**
 * Checks whether an OSC message address matches a given wildcard expression, as
 * specified in the OSC protocol specification. For details, see the "OSC
 * Message Dispatching and Pattern Matching" section on
 * <a href="http://opensoundcontrol.org/spec-1_0">the OSC 1.0 specification</a>.
 * Also supports the path-traversal wildcard "//", as specified in OSC 1.1
 * (borrowed from XPath).
 *
 * <p>
 * A coarse history of the code in the function
 * {@link OSCPatternAddressSelector#matches(String, String)}, from the origin to
 * JavaOSC:
 * </p>
 * <ol>
 * <li><b>robust glob pattern matcher</b><br>
 * language: <i>C</i><br>
 ...
 * </li>
 * <li><b>Open SoundControl kit</b><br>
 * language: <i>C++</i><br>
 * matching code license: <i>public domain</i><br>
 * library license: <i>LGPL 2.1+</i><br>
 * <a href=
 * "http://archive.cnmat.berkeley.edu/OpenSoundControl/src/OSC-Kit/OSC-pattern-match.c">source
 * location</a><br>
 * </li>
 * <li><b>LibLO</b><br>
 * language: <i>C++</i><br>
 * library license: <i>LGPL 2.1+</i><br>
 * <a href=
 * "https://sourceforge.net/p/liblo/git/ci/master/tree/src/pattern_match.c">source
 * location</a><br>
 * </li>
 * <li><b>JavaOSC</b><br>
...
public class OSCPatternAddressSelector implements AddressSelector {
	...
	/**
	 * Splits an OSC message address or address selector pattern into parts that
	 * are convenient during the matching process.
	 * 
	 * @param addressOrPattern
	 *            to be split into parts, e.g.: "/hello/", "/hello//world//"
	 * @return the given address or pattern split into parts: {"hello"},
	 *         {"hello, "", "world", ""}
	 */
	public static List<String> splitIntoParts(String addressOrPattern) {
		...
		if (addressOrPattern.startsWith("/")) {
			// as "/hello" gets split into {"", "hello"}, we remove the first
			// empty entry,
			// so we end up with {"hello"}
			parts.remove(0);
		}
		if (addressOrPattern.endsWith("/")) {
			// as "hello/" gets split into {"hello", ""}, we also remove the
			// last empty entry,
			// so we end up with {"hello"}
	...
	/**
	 * Tries to match an OSC <i>Address Pattern</i> to a selector, both already
	 * divided into their parts.
	 * 
	 * @param patternParts
	 *            all the parts of the pattern
	 * @param ppi
	 *            index/pointer to the current part of the pattern we are
	 *            looking at
	 * @param messageAddressParts
	 *            all the parts of the address
	 * @param api
	 *            index/pointer to the current part of the address we are
	 *            looking at
	...
	private static boolean matches(List<String> patternParts, int ppi, List<String> messageAddressParts, int api) {
		...
		while (ppi < patternParts.size()) {
			// There might be some path-traversal wildcards (PTW) "//" in the
			// pattern.
			// "//" in the pattern translates to an empty String ("") in the
			// pattern parts.
			...
			if (pathTraverser) {
				...
				// end of address parts reached, but there are still non-PTW
				// pattern parts
				// left
				return false;
			} else {
				if ((ppi == patternParts.size()) != (api == messageAddressParts.size())) {
					// end of pattern, no trailing PTW, but there are still
					// address parts left
					// OR
					// end of address, but there are still non-PTW pattern parts
					// left
					return false;
				}
				if (!matches(messageAddressParts.get(api), patternParts.get(ppi))) {
	...
	/**
	 * Tries to match an OSC <i>Address Pattern</i> part to a part of a
	 * selector. This code was copied and adapted from LibLo, and is licensed
	 * under the Public Domain. For more details see:
	 * {@link OSCPatternAddressSelector}.
	 * 
	 * @param str
	 *            address part
	 * @param p
	...
	private static boolean matches(String str, String p) {
		...
		while (pi < p.length()) {
			 ...
			 * everything in between. this means [z-a] may be interpreted as a
			 * set that contains z, a and nothing in between.
			 */
			case '[':
				if (p.charAt(pi) == '!') {
					negate = true;
			...
			/*
			 * Not part of OSC pattern matching case '\\': if (p.charAt(pi)) { c
			 * = p.charAt(pi)++; }
			 */
			default:
				if (c != str.charAt(si)) {
